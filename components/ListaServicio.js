            ...
            if (emp.tipo === 1) {
              return (
                <Fragment key={emp._id}>
                  <Card>
                    <CardItem button onPress={() => navigation.navigate('FormTanque', { idEmp: emp._id, empresa: emp.nombre })}>
                      <Grid>
                        <Col size={35}><Thumbnail square source={{ uri: URL_FILE + emp.logo }} style={styles.img} /></Col>
                        <Col size={65}>
                          <Text style={styles.title}>Gas / Tanque</Text>
                          <Text style={{fontSize: 15, color: 'gray', fontStyle: 'italic' }}>{emp.nombre}</Text>
                  ...
                  <Card>
                    <CardItem button onPress={() => navigation.navigate('FormEstacionario', { idEmp: emp._id, empresa: emp.nombre })}>
                      <Grid>
                        <Col size={35}><Thumbnail square source={{ uri: URL_FILE + emp.logo }} style={styles.img} /></Col>
                        <Col size={65}>
              ...
              return (
                <Card key={emp._id}>
                  <CardItem button onPress={() => navigation.navigate('FormAgua', { idEmp: emp._id, empresa: emp.nombre })}>
                    <Grid>
                      <Col size={35}><Thumbnail square source={{ uri: URL_FILE + emp.logo }} style={styles.img} /></Col>
                      <Col size={65}>
